...
public class PageTurnerPrefsActivity extends PreferenceActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (!Configuration.IS_NOOK_TOUCH) {
			...
			// Nook Touch can't render OTF fonts (causes segfault), also most
			// thin weighted fonts look terrible
			// because the Nook Touch libskia uses antialiasing, but then the
			// Nook Touch screen can't adequately
			// render any antialiasing...
			final String[] font_prefs = { "font_face", "serif_font",
					"sans_serif_font" };
			for (String font_pref : font_prefs) {
				ListPreference pref = (ListPreference) screen
						.findPreference(font_pref);
				pref.setEntries(getResources().getStringArray(
						R.array.builtinFontLabels));
				pref.setEntryValues(getResources().getStringArray(
						R.array.builtinFonts));
			}
		}

		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {

			Preference uiPref = screen
					.findPreference(Configuration.KEY_DIM_SYSTEM_UI);
			PreferenceGroup group = (PreferenceGroup) screen
					.findPreference("visual_prefs");
