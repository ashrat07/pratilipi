...
import com.pratilipi.android.fragment.ReadingFragment;
...
public class ReadingActivity extends PageTurnerActivity {
	...
	protected void initDrawerItems(ExpandableListView expandableListView) {
		...
		if (this.readingFragment != null) {
			...
			if (readingFragment.hasSearchResults()) {
				final List<NavigationCallback> searchCallbacks = this.readingFragment
						.getSearchResults();
				...
				getAdapter().findGroup(this.searchIndex).match(
						new OptionMatcher<NavigationCallback>() {

							@Override
							public void caseNone() {
								// Could not find Search drawer item!
							}

							@Override
							public void caseSome(final NavigationCallback s) {
								forEach(searchCallbacks,
										new Command<NavigationCallback>() {

											@Override
											public void execute(
													NavigationCallback child) {
												s.addChild(child);
											}
										});
							}
	...
	protected List<NavigationCallback> getMenuItems(Configuration config) {
		...
		// menuItems.add( new
		// NavigationCallback(getString(R.string.open_library))
		...
		menuItems.add(new NavigationCallback(getString(R.string.prefs))
				.setOnClick(new UiUtils.Action() {
