	...
	public void setFolder(File folder) {
		...
		if (listing != null) {
			for (File childFile : listing) {
				if (childFile.isDirectory()
						|| childFile.getName().toLowerCase(Locale.US)
								.endsWith(".epub")) {
		...
		Collections.sort(items, new Comparator<FileItem>() {
			...
			public int compare(FileItem lhs, FileItem rhs) {
				if ((lhs.getFile().isDirectory() && rhs.getFile().isDirectory())
						|| (!lhs.getFile().isDirectory() && !rhs.getFile()
								.isDirectory())) {
					return lhs.getFile().getName()
							.compareTo(rhs.getFile().getName());
				}
		...
		items.add(0, new FileItem("[" + context.getString(R.string.import_this)
				+ "]", folder, true));
		...
		selectBox
				.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
					...
					public void onCheckedChanged(CompoundButton buttonView,
							boolean isChecked) {
						if (isChecked && itemSelectionListener != null) {
							itemSelectionListener.fileSelected(fileItem
									.getFile());
						}
					}
				});
