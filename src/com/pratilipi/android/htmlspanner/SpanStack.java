...
public class SpanStack {
	...
	public Style getStyle(TagNode node, Style baseStyle) {
		...
		if (!lookupCache.containsKey(node)) {
			...
			Log.v("SpanStack",
					"Looking for matching CSS rules for node: " + "<"
							+ node.getName() + " id='"
							+ option(node.getAttributeByName("id"))
							+ "' class='"
							+ option(node.getAttributeByName("class")) + "'>");
			...
			Log.v("SpanStack", "Found " + matchingRules.size()
					+ " matching rules.");
	...
	public void pushSpan(final Object span, final int start, final int end) {
		...
		if (end > start) {
			SpanCallback callback = new SpanCallback() {
				@Override
				public void applySpan(HtmlSpanner spanner,
			...
			Log.d("SpanStack", "refusing to put span of type "
					+ span.getClass().getSimpleName() + " and length "
					+ (end - start));
		}
	}
