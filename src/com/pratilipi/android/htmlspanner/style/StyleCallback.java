package com.pratilipi.android.htmlspanner.style;
...
import com.pratilipi.android.htmlspanner.SpanCallback;
import com.pratilipi.android.htmlspanner.spans.AlignNormalSpan;
import com.pratilipi.android.htmlspanner.spans.AlignOppositeSpan;
...
public class StyleCallback implements SpanCallback {
	...
	public void applySpan(HtmlSpanner spanner, SpannableStringBuilder builder) {
		...
		if (useStyle.getFontFamily() != null || useStyle.getFontStyle() != null
			...
			// Log.d("StyleCallback", "Applying FontFamilySpan from " + start +
			// " to " + end + " on text " + builder.subSequence(start, end));
		...
		// If there's no border, we use a BackgroundColorSpan to draw colour
		// behind the text
		if (spanner.isUseColoursFromStyle()
			...
			// + useStyle.getBackgroundColor() + " from " + start + " to " + end
			// + " on text " + builder.subSequence(start, end));
			builder.setSpan(
					new BackgroundColorSpan(useStyle.getBackgroundColor()),
					start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		}

		// If there is a border, the BorderSpan will also draw the background
		// colour if needed.
		if (useStyle.getBorderStyle() != null) {
			builder.setSpan(
					new BorderSpan(useStyle, start, end, spanner
							.isUseColoursFromStyle()), start, end,
					Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
		...
		if (useStyle.getFontSize() != null) {
			...
			if (styleValue.getUnit() == StyleValue.Unit.PX) {
				if (styleValue.getIntValue() > 0) {
					// Log.d("StyleCallback",
					// "Applying AbsoluteSizeSpan with size " +
					// useStyle.getAbsoluteFontSize() + " from " + start +
					// " to " + end + " on text " + builder.subSequence(start,
					// end));
					builder.setSpan(
				...
				if (styleValue.getFloatValue() > 0f) {
					// Log.d("StyleCallback",
					// "Applying RelativeSizeSpan with size " +
					// useStyle.getRelativeFontSize() + " from " + start +
					// " to " + end + " on text " + builder.subSequence(start,
					// end));
					builder.setSpan(
		...
		if (spanner.isUseColoursFromStyle() && useStyle.getColor() != null) {
			// Log.d("StyleCallback", "Applying ForegroundColorSpan from " +
			// start + " to " + end + " on text " + builder.subSequence(start,
			// end) );
			builder.setSpan(new ForegroundColorSpan(useStyle.getColor()),
					start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		}
		...
		if (useStyle.getTextAlignment() != null) {
			...
			// Log.d("StyleCallback", "Applying AlignmentSpan from " + start +
			// " to " + end + " on text " + builder.subSequence(start, end) );
		...
		if (useStyle.getTextIndent() != null) {
			...
			Log.d("StyleCallback",
					"Applying LeadingMarginSpan from " + marginStart + " to "
							+ marginEnd + " on text "
			...
			if (styleValue.getUnit() == StyleValue.Unit.PX) {
				if (styleValue.getIntValue() > 0) {
					builder.setSpan(
							new LeadingMarginSpan.Standard(styleValue
									.getIntValue(), 0), marginStart, marginEnd,
							Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
				}
				...
				if (styleValue.getFloatValue() > 0f) {
					builder.setSpan(
							new LeadingMarginSpan.Standard(
									(int) (HtmlSpanner.HORIZONTAL_EM_WIDTH * styleValue
											.getFloatValue()), 0), marginStart,
							marginEnd, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
				}
			}
		...
		/*
		 * We ignore negative horizontal margins, since that would cause the
		 * text to be rendered off-screen
		 */
	...
	/**
	 * Returns the current FontFamilySpan in use on the given subsection of the
	 * builder.
	 * 
	 ...
	 */
	private FontFamilySpan getFontFamilySpan(SpannableStringBuilder builder,
			int start, int end) {

		FontFamilySpan[] spans = builder.getSpans(start, end,
				FontFamilySpan.class);
