...
public class StyleValue {
	...
	public static StyleValue parse(String value) {
		...
		if (value.endsWith("px")) {
			...
			try {
				final Integer intValue = Integer.parseInt(value.substring(0,
						value.length() - 2));
		...
		if (value.endsWith("%")) {
			...
			try {
				final int percentage = Integer.parseInt(value.substring(0,
						value.length() - 1));
		...
		if (value.endsWith("em")) {
			try {
				final Float number = Float.parseFloat(value.substring(0,
						value.length() - 2));
