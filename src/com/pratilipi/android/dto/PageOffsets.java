 ...
 */
package com.pratilipi.android.dto;
 ...
 * 
 * Page-offsets are only valid under the circumstances they were calculated
 * with: if text-size, page-margins, etc. change, they must be re-calculated.
 * This class allows checks for this.
 * 
 * @author Alex Kuiper
 * 
 */
public class PageOffsets {
	...
	public static PageOffsets fromValues(Configuration config,
	...
	public static PageOffsets fromJSON(String json) throws JSONException {
		...
		result.algorithmVersion = offsetsObject.optInt(
				Fields.algorithmVersion.name(), -1);
		result.allowStyling = offsetsObject.optBoolean(
				Fields.allowStyling.name(), true);

		result.offsets = readOffsets(offsetsObject.getJSONArray(Fields.offsets
				.name()));
	...
	private static List<List<Integer>> readOffsets(JSONArray jsonArray)
