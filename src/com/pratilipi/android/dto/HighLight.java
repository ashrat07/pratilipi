...
/**
 * Created with IntelliJ IDEA. User: alex Date: 6/17/13 Time: 11:54 AM To change
 * this template use File | Settings | File Templates.
 */
public class HighLight {

	private static enum Fields {
		displayText, textNote, index, start, end, color
	};
	...
	public HighLight(String displayText, int index, int start, int end,
			int color) {
		this.start = start;
		this.end = end;
		this.index = index;
	...
	public static String toJSON(List<HighLight> highLights) {
		try {
			...
			for (HighLight highLight : highLights) {
				...
				jsonObject.put(Fields.displayText.name(),
						highLight.getDisplayText());

				if (highLight.getTextNote() != null) {
					jsonObject.put(Fields.textNote.name(),
			...
			throw new IllegalArgumentException("Could not serialize to json",
					json);
		}
	}
