	...
	public Option<Void> doInBackground(String... params) {
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == 200) {
				...
				/**
				 * Make sure we always store downloaded files as .epub, so they
				 * show up in scans later on.
				 */
				...
				if (!isCancelled()) {
					...
					libraryService.storeBook(destFile.getAbsolutePath(), book,
							false, config.getCopyToLibraryOnScan());
				}

			} else {
				this.failure = new RuntimeException(response.getStatusLine()
						.getReasonPhrase());
			}
