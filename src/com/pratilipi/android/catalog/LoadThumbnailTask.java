...
public class LoadThumbnailTask extends
		QueueableAsyncTask<Link, Void, FastBitmapDrawable> {
	...
	public Option<FastBitmapDrawable> doInBackground(Link... entries) {
		...
		if (imageLink != null) {
			...
			try {
				...
				Bitmap bitmap = BitmapFactory.decodeStream(resp.getEntity()
						.getContent());
	...
	public void doOnPostExecute(Option<FastBitmapDrawable> drawable) {
		...
		drawable.forEach(new Command<Drawable>() {

			public void execute(Drawable d) {
				callBack.notifyLinkUpdated(imageLink, d);
			};
		});
	}
}
