...
import com.pratilipi.android.nucular.parser.Nucular;
import com.pratilipi.android.nucular.parser.opensearch.SearchDescription;
...
public class LoadOPDSTask extends QueueableAsyncTask<String, Object, Feed> {
	...
	public Option<Feed> doInBackground(String... params) {
		...
		try {
			...
			currentRequest.setHeader("Accept-Language", config.getLocale()
					.getLanguage());
			HttpResponse response = httpClient.execute(currentRequest);

			if (response.getStatusLine().getStatusCode() != 200) {
				this.errorMessage = "HTTP "
			...
			Option<Link> searchLinkOption = feed
					.findByRel(AtomConstants.REL_SEARCH);

			if (!isEmpty(searchLinkOption)) {

				final Link searchLink = searchLinkOption.unsafeGet();
				...
				if (AtomConstants.TYPE_OPENSEARCH.equals(searchLink.getType())) {
					...
					try {
						...
						// );
						desc.getSearchLink().forEach(new Command<Link>() {
	...
	public void doOnPostExecute(Option<Feed> result) {
		...
		result.match(new OptionMatcher<Feed>() {
	...
	private void addCustomSitesEntry(Feed feed) {
		...
		// iconLink.forEach( l -> {
		// Link thumbnailLink = new Link(l.getHref(), l.getType(),
		// AtomConstants.REL_IMAGE, null);
		// entry.addLink(thumbnailLink);
		// });
		iconLink.forEach(new Command<Link>() {
