...
import static com.pratilipi.android.catalog.Catalog.getImageLink;
...
public class CatalogListAdapter extends BaseAdapter {
	...
	public void setLoading(boolean loading) {
		if (loading) {
			if (feed.getSize() > 0
					&& !feed.getEntries().contains(this.loadingEntry)) {

				Entry lastEntry = feed.getEntries().get(feed.getSize() - 1);
				feed.addEntry(this.loadingEntry);

				// lastEntry.getFeed().forEach( f ->
				// this.loadingEntry.setFeed(f) );
				lastEntry.getFeed().forEach(new Command<Feed>() {

					public void execute(Feed f) {
						loadingEntry.setFeed(f);
					};
				});
			}
		} else {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		icon.setImageDrawable(drawableOption.getOrElse(context.getResources()
	...
	private Option<Drawable> getThumbnail(Entry entry) {
		...
		return this.imageLoader.getThumbnailFor(entry.getBaseURL(),
				imgLink.unsafeGet());
	}
