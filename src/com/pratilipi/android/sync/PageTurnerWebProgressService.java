 ...
 */
package com.pratilipi.android.sync;
...
public class PageTurnerWebProgressService implements ProgressService {
	...
	@Override
	public Option<List<BookProgress>> getProgress(String fileName)
		...
		if ("".equals(userId) || "".equals(fileName)) {
			// Empty username or filename. Aborting sync.
			return none();
		}

		String key = computeKey(fileName);

		HttpGet get = new HttpGet(getSyncServerURL() + key + "?accessKey="
				+ URLEncoder.encode(accessKey));
		...
		try {
			...
			if (statusCode == HTTP_NOT_FOUND) {
				List<BookProgress> list = new ArrayList<BookProgress>();
				return option(list);
			}
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				result.add(new BookProgress(fileName, index, progress,
						percentage, timeStamp, deviceName));
			}

			return some(result);

		} catch (IOException e) {
			// Got error while querying server
		} catch (JSONException json) {
			// Error reading response
		} catch (ParseException p) {
			// Invalid date
		} catch (IllegalStateException s) {
			// Tried query in illegal state
		}
	...
	public void storeProgress(String fileName, int index, int progress,
		...
		try {
			...
			pairs.add(new BasicNameValuePair("title", Integer
					.toHexString(filePart.hashCode())));
			pairs.add(new BasicNameValuePair("deviceName", this.config
					.getDeviceName()));
			pairs.add(new BasicNameValuePair("percentage", "" + percentage));
			pairs.add(new BasicNameValuePair("userId", Integer
					.toHexString(this.config.getSynchronizationEmail()
							.hashCode())));
			pairs.add(new BasicNameValuePair("accessKey", this.config
					.getSynchronizationAccessKey()));
	...
	private String computeKey(String fileName) {
		...
		String plainTextKey = this.config.getSynchronizationEmail() + ":"
				+ filePart;
