 ...
 */
package com.pratilipi.android.nucular.atom;
...
public class Entry extends AtomElement {
	...
	public Option<Link> getAlternateLink() {
		...
		// return atomLink.filter(l -> l.getRel() != null &&
		// l.getRel().equalsIgnoreCase(REL_ALTERNATE));
		return atomLink.filter(new Filter<Link>() {

			@Override
			public Boolean execute(Link l) {
				return l.getRel() != null
						&& l.getRel().equalsIgnoreCase(
								AtomConstants.REL_ALTERNATE);
			}
		});
	}
	...
	public Option<Link> getAtomLink() {
		...
		return firstOption(links, new Filter<Link>() {
			...
			@Override
			public Boolean execute(Link l) {
				return l.getType().startsWith(AtomConstants.TYPE_ATOM);
			}
		});
	}
	...
	public List<Link> getAlternateLinks() {
		...
		for (Link link : getLinks()) {
			...
			if (rel.equals(AtomConstants.REL_RELATED)
					&& type.startsWith(AtomConstants.TYPE_ATOM)) {
				result.add(link);
			}
		}
	...
	public Option<Link> getThumbnailLink() {
		return findByRel(AtomConstants.REL_THUMBNAIL,
				AtomConstants.REL_THUMBNAIL_ALT,
				AtomConstants.REL_STANZA_THUMBNAIL_IMAGE);
	}

	public Option<Link> getImageLink() {
		return findByRel(AtomConstants.REL_IMAGE, AtomConstants.REL_COVER,
				AtomConstants.REL_STANZA_COVER_IMAGE);
	}
	...
	public Option<Link> getEpubLink() {
		// return firstOption( getLinks(), link -> link.getType() != null &&
		// link.getType().equals(TYPE_EPUB));
		return firstOption(getLinks(), new Filter<Link>() {

			@Override
			public Boolean execute(Link link) {
				return link.getType() != null
						&& link.getType().equals(AtomConstants.TYPE_EPUB);
			}
		});
	}
}
