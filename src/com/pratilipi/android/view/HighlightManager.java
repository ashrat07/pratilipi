...
public class HighlightManager {
	...
	public synchronized void registerHighlight(String bookFile,
			String displayText, int index, int start, int end) {

		updateBookFile(bookFile);

		currentHighlights.add(new HighLight(displayText, index, start, end,
				Color.YELLOW));
		saveHighLights();
	}
	...
	public synchronized void saveHighLights() {
		if (currentFileName != null && currentHighlights != null) {
			Log.d("HighlightManager", "Storing highlights for file "
					+ currentFileName + ": " + currentHighlights.size()
					+ " items.");
