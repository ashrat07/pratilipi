...
package com.pratilipi.android.view;
...
import com.pratilipi.android.view.bookview.HighlightSpan;
...
public class NavGestureDetector extends GestureDetector.SimpleOnGestureListener {
	...
	public boolean onSingleTapUp(MotionEvent e) {
		...
		List<HighlightSpan> highlightSpans = bookView.getHighlightsAt(e.getX(),
				e.getY());

		Log.d("NavGestureDetector", "Got " + highlightSpans.size()
				+ " HighLightSpans.");
	...
	public void onLongPress(MotionEvent e) {
		...
		if (Build.VERSION.SDK_INT < Configuration.TEXT_SELECTION_PLATFORM_VERSION) {
			Option<SelectedWord> wordOption = bookView.getWordAt(e.getX(),
					e.getY());

			// wordOption.match( word ->
			// bookViewListener.onWordLongPressed( word.getStartOffset(),
			// word.getEndOffset(), word.getText() )
			// , () -> {} );
			wordOption.match(new OptionMatcher<SelectedWord>() {

				@Override
				public void caseSome(SelectedWord word) {
					bookViewListener.onWordLongPressed(word.getStartOffset(),
							word.getEndOffset(), word.getText());
				}
