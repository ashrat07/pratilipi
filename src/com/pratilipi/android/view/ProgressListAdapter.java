...
/**
 * ListAdapter class for progress downloaded from a synchronization server.
 * 
 * @author Alex Kuiper
 * 
 */
public class ProgressListAdapter extends ArrayAdapter<BookProgress> implements
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		int progressPercentage = bookView.getPercentageFor(progress.getIndex(),
				progress.getProgress());
		int pageNumber = bookView.getPageNumberFor(progress.getIndex(),
				progress.getProgress());
		...
		if (pageNumber != -1) {
			text = String
					.format(getContext().getString(R.string.page_number_of),
							pageNumber, totalPages)
					+ " (" + progressPercentage + "%)";
		}

		deviceView.setText(progress.getDeviceName() + " - " + text);
		dateView.setText(SimpleDateFormat.getDateTimeInstance().format(
