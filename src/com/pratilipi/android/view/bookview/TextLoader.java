...
public class TextLoader implements LinkTagHandler.LinkCallBack {
	...
	public void setHtmlSpanner(HtmlSpanner spanner) {
		...
		spanner.registerHandler("a", registerAnchorHandler(new LinkTagHandler(
				this)));
	...
	public void registerCustomFont(String name, String href) {
		...
		// Registering custom font with href
	...
	public Book initBook(String fileName) throws IOException {
		...
		if (hasCachedBook(fileName)) {
			// Returning cached Book for fileName
			return currentBook;
		}
	...
	public Spannable getText(final Resource resource,
		...
		if (memoryUsage > CACHE_CLEAR_THRESHOLD
				|| bitmapUsage > CACHE_CLEAR_THRESHOLD) {
		...
		if (!resource.isInitialized()) {
			// TODO
			// res = new Resource(this.currentFile, res.getSize(),
			// res.getOriginalHref());
		...
		try {
			result = htmlSpanner
					.fromHtml(res.getReader(), cancellationCallback);
			renderedText.put(res.getHref(), result);
		} catch (Exception e) {
			// Caught exception while rendering text
			result = new SpannableString(e.getClass().getSimpleName() + ": "
					+ e.getMessage());
		} finally {
			if (shouldClose) {
				// We have the rendered version, so it's safe to close the
				// resource
				resource.close();
			}
		}
