...
public class FixedPagesStrategy implements PageChangeStrategy {
	...
	public List<Integer> getPageOffsets(CharSequence text,
		...
		StaticLayout layout = layoutFactory.create(text, textPaint,
				boundedWidth, bookView.getLineSpacing());
		...
		int pageHeight = bookView.getMeasuredHeight()
				- bookView.getVerticalMargin();

		if (includePageNumbers) {
			String bottomSpace = "0\n";

			StaticLayout numLayout = layoutFactory.create(bottomSpace,
					textPaint, boundedWidth, bookView.getLineSpacing());
		...
		while (topLineNextPage < totalLines - 1) {
			...
			topLineNextPage = layout.getLineForVertical(layout
					.getLineTop(topLine) + pageHeight);

			if (topLineNextPage == topLine) { // If lines are bigger than can
												// fit on a page
			...
			if (pageEnd > pageStartOffset) {
				if (text.subSequence(pageStartOffset, pageEnd).toString()
						.trim().length() > 0) {
	...
	public void updatePosition() {
		...
		if (sequence.length() > 0) {
			...
			// #555 Remove \n at the end of sequence which get InnerView size
			// changed
	...
	private Option<CharSequence> getTextForPage(int page) {
		...
		if (pageOffsets.size() < 1 || page < 0) {
			...
			if (startOffset >= 0 && startOffset <= text.length() - 1) {
				return some(applySpans(
						this.text.subSequence(startOffset, text.length()),
						startOffset));
			} else {
	...
	private CharSequence applySpans(CharSequence text, int offset) {
		List<HighLight> highLights = highlightManager.getHighLights(bookView
				.getFileName());
	...
	public int getProgressPosition() {
		...
		if (storedPosition > 0 || this.pageOffsets.isEmpty()
				|| this.pageNum == -1) {
