...
public class ScrollingStrategy implements PageChangeStrategy {
	...
	private Spannable addEndTag(SpannableStringBuilder builder) {
		...
		builder.setSpan(new ImageSpan(img), length, length + 1,
				Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		builder.setSpan(span, length, builder.length(),
				Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
	...
	private void scroll(int delta) {
		...
		if (bookView.getScrollY() == currentPos) {
			...
			if (delta < 0) {
				if (bookView.getSpine() == null
						|| !bookView.getSpine().navigateBack()) {
					return;
				}
			} else {
				if (bookView.getSpine() == null
						|| !bookView.getSpine().navigateForward()) {
					return;
				}
			}
