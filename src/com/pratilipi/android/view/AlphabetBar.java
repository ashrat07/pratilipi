...
public class AlphabetBar extends LinearLayout {
	...
	public AlphabetBar(Context context, AttributeSet attrs) {
		...
		ColourProfile profile = RoboGuice.getInjector(context)
				.getInstance(Configuration.class).getColourProfile();
	...
	public void init() {
		 ...
		 * our custom logic, but it seems like good form to do this just to show
		 * how we're arranging the children.
		 */
	...
	protected void onMeasure(int wSpec, int hSpec) {
		...
		for (int i = 0; i < count; i++) {
			...
			label.measure(MeasureSpec.makeMeasureSpec(26, MeasureSpec.EXACTLY),
					MeasureSpec
							.makeMeasureSpec(thisHeight, MeasureSpec.EXACTLY));
