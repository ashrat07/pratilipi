...
public class HttpGet extends
	...
	protected JSONObject doInBackground(HashMap<String, String>... params) {
		...
		try {
			...
			LoggerUtils.logWarn("HTTP GET Request", url.toString());
			conn.connect();
			try {
				mResponseCode = conn.getResponseCode();
				LoggerUtils.logWarn("HTTP GET Response Code:", ""
						+ mResponseCode);
				...
				LoggerUtils.logWarn("Exception", Log.getStackTraceString(e));
				LoggerUtils.logWarn("HTTP GET Response Code(2):", ""
						+ mResponseCode);
				return null;
			}
			...
			if (conn.getInputStream() != null) {
				...
				try {
					...
					LoggerUtils.logWarn("HTTP Get JsonException",
							Log.getStackTraceString(js));
				}

				if (finalResult != null) {
					LoggerUtils.logWarn("HTTP Get Response",
							finalResult.toString());
				}

			} else {
				LoggerUtils.logWarn("HTTP Get Response", "Nothing");
			}
