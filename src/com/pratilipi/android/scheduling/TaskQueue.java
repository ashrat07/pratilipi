package com.pratilipi.android.scheduling;
 ...
 * 
 * Allows for consistent execution and cancelling of tasks across Android
 * versions.
 * 
 * @author Alex Kuiper
 */
public class TaskQueue {
	...
	public <A, B, C> void executeTask(QueueableAsyncTask<A, B, C> task,
			A... parameters) {
		...
		task.setCallback(new QueueableAsyncTask.QueueCallback() {
			...
			@Override
			public void taskCompleted(QueueableAsyncTask<?, ?, ?> task,
					boolean wasCancelled) {
				TaskQueue.this.taskCompleted(task, wasCancelled);
			}
		});
		...
		if (this.taskQueue.size() == 1) {
			Log.d("TaskQueue", "Starting task " + taskQueue.peek()
					+ " since task queue is 1.");
			this.taskQueue.peek().execute();
		}
	}

	/**
	 * Cancels the currently running task and queues this task ahead of all
	 * others.
	 * 
	 * @param task
	 * @param parameters
	 ...
	 * @param <C>
	 */
	public <A, B, C> void jumpQueueExecuteTask(
			QueueableAsyncTask<A, B, C> task, A... parameters) {

		Log.d("TaskQueue", "Queue-jump requested for "
				+ task.getClass().getSimpleName());

		if (this.taskQueue.isEmpty()) {
			Log.d("TaskQueue",
					"Delegating to simple schedule since the queue is empty.");
	...
	private Option<QueuedTask<?, ?, ?>> findQueuedTaskFor(
			QueueableAsyncTask<?, ?, ?> task) {
		for (QueuedTask<?, ?, ?> wrapper : this.taskQueue) {
			if (wrapper.getTask() == task) {
				// return some(wrapper);
				return new Some<QueuedTask<?, ?, ?>>(wrapper);
			}
		}
	...
	public void taskCompleted(QueueableAsyncTask<?, ?, ?> task,
		...
		if (!this.taskQueue.isEmpty()) {
			...
			if (!this.taskQueue.peek().isExecuting()) {
				...
				Log.d("TaskQueue",
						"Task at the head of queue is already running.");
			}
