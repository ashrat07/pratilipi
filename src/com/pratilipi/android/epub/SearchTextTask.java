...
public class SearchTextTask extends
		QueueableAsyncTask<String, SearchResult, List<SearchResult>> {
		...
		try {
			...
			for (int index = 0; index < spine.size(); index++) {
				...
				if (!isEmpty(currentResource)) {
					...
					Spanned spanned = spanner.fromHtml(currentResource
							.unsafeGet().getReader());
					...
					while (matcher.find()) {
						...
						String text = "???"
								+ spanned.subSequence(from, to).toString()
										.trim() + "???";
						SearchResult res = new SearchResult(searchTerm, text,
