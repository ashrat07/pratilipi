 ...
 */
package com.pratilipi.android.epub;
...
public class PageTurnerSpine implements Iterable<PageTurnerSpine.SpineEntry> {
	...
	/**
	 * Returns the number of entries in this spine. This includes the generated
	 * cover.
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the title of the current entry, or null if it could not be
	 * determined.
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the current resource, or null if there is none.
	 * 
	 * @return
	 */
	...
	private static String resolveHref(String href, String against) {
		try {
			String result = new URI(encode(against)).resolve(encode(href))
					.getPath();
			return result;
	...
	private static String encode(String input) {
		...
		for (char ch : input.toCharArray()) {
			if (ch == '\\') { // Some books use \ as a separator... invalid, but
								// we'll try to fix it
				resultStr.append('/');
			} else if (isUnsafe(ch)) {
				resultStr.append('%');
	...
	/**
	 * This is slightly unsafe: it lets / and % pass, making multiple encodes
	 * safe.
	 * 
	 * @param ch
	 * @return
	...
	/**
	 * Returns a percentage, which indicates how far the given point in the
	 * current entry is compared to the whole book.
	 * 
	...
	/**
	 * Returns the progress percentage for the given text position in the given
	 * index.
	 * 
	 * @param index
	 * @param position
	...
	public int getProgressPercentage(int index, int position) {
		...
		double progressInPart = ((double) position / (double) entries
				.get(index).size);
		return getProgressPercentage(index, progressInPart);
	}

	/**
	 * Returns a list of doubles representing the relative size of each spine
	 * index.
	 * 
	 * @return
	 */
	...
	private Resource createCoverResource(Book book) {
		...
		if (book.getCoverPage() != null && book.getCoverPage().getSize() > 0
			...
			Log.d("PageTurnerSpine", "Using cover resource "
					+ book.getCoverPage().getHref());

			return book.getCoverPage();
		}

		Log.d("PageTurnerSpine", "Constructing a cover page");
		Resource res = new Resource(generateCoverPage(book).getBytes(),
				COVER_HREF);
		res.setTitle("Cover");
		...
		if (book.getCoverImage() == null) {
			centerpiece = "<center><h1>"
					+ (book.getTitle() != null ? book.getTitle()
							: "Book without a title") + "</h1>";

			if (!book.getMetadata().getAuthors().isEmpty()) {
				for (Author author : book.getMetadata().getAuthors()) {
					centerpiece += "<h3>" + author.getFirstname() + " "
							+ author.getLastname() + "</h3>";
				}
			} else {
