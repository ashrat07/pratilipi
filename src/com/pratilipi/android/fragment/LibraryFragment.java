...
import com.pratilipi.android.activity.CatalogActivity;
import com.pratilipi.android.activity.FileBrowseActivity;
import com.pratilipi.android.activity.LibraryActivity;
import com.pratilipi.android.activity.PageTurnerActivity;
import com.pratilipi.android.activity.ReadingActivity;
import com.pratilipi.android.htmlspanner.HtmlSpanner;
import com.pratilipi.android.library.CleanFilesTask;
import com.pratilipi.android.library.ImportCallback;
import com.pratilipi.android.library.ImportTask;
import com.pratilipi.android.library.KeyedQueryResult;
import com.pratilipi.android.library.KeyedResultAdapter;
import com.pratilipi.android.library.LibraryBook;
import com.pratilipi.android.library.LibraryService;
import com.pratilipi.android.library.QueryResult;
import com.pratilipi.android.scheduling.QueueableAsyncTask;
import com.pratilipi.android.scheduling.TaskQueue;
import com.pratilipi.android.util.Configuration;
import com.pratilipi.android.util.Configuration.ColourProfile;
import com.pratilipi.android.util.Configuration.LibrarySelection;
import com.pratilipi.android.util.Configuration.LibraryView;
...
import com.pratilipi.android.view.BookCaseView;
import com.pratilipi.android.view.FastBitmapDrawable;
...
import static com.pratilipi.android.util.UiUtils.onCollapse;
import static com.pratilipi.android.util.UiUtils.onMenuPress;
import static com.pratilipi.android.util.PlatformUtil.isIntentAvailable;
...
public class LibraryFragment extends Fragment implements ImportCallback {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Bitmap backupBitmap = BitmapFactory.decodeResource(getResources(),
		...
		if (savedInstanceState != null) {
			this.askedUserToImport = savedInstanceState.getBoolean("import_q",
					false);
		}

		// this.taskQueue.setTaskQueueListener(this::onTaskQueueEmpty);
		this.taskQueue.setTaskQueueListener(new TaskQueue.TaskQueueListener() {
		...
		this.listView
				.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						LibraryFragment.this.onItemClick(parent, view,
								position, id);
					}
				});
		...
		this.listView
				.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
					...
					public boolean onItemLongClick(AdapterView<?> parent,
						...
						return LibraryFragment.this.onItemLongClick(parent,
								view, position, id);
					}
				});
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		actionBar.setListNavigationCallbacks(adapter,
				new ActionBar.OnNavigationListener() {
					...
					public boolean onNavigationItemSelected(int itemPosition,
							long itemId) {
						return LibraryFragment.this.onNavigationItemSelected(
								itemPosition, itemId);
					}
				});
		...
		// executeTask(new ImportTask(getActivity(), libraryService, this,
		// config, config.getCopyToLibraryOnScan(),
		// true), folder );
		// }, () -> {
		// LOG.error("No library folder present!");
		// Toast.makeText( context, R.string.library_failed, Toast.LENGTH_LONG
		// ).show();
		// });
		libraryFolder.match(new OptionMatcher<File>() {

			@Override
			public void caseSome(File folder) {
				executeTask(new CleanFilesTask(libraryService,
						new CleanFilesTask.DeleteBooksCallback() {

							@Override
							public void booksDeleted(int numberOfDeletedBooks) {
								booksDeleted(numberOfDeletedBooks);
							}
						}));
				executeTask(
						new ImportTask(getActivity(), libraryService,
								LibraryFragment.this, config, config
										.getCopyToLibraryOnScan(), true),
						folder);
			}

			@Override
			public void caseNone() {
				Toast.makeText(context, R.string.library_failed,
	...
	private <A, B, C> void executeTask(QueueableAsyncTask<A, B, C> task,
			A... parameters) {
	...
	private void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if (config.getLongShortPressBehaviour() == Configuration.LongShortPressBehaviour.NORMAL) {
			// this.bookAdapter.getResultAt( position ).forEach(
			// this::showBookDetails );
			this.bookAdapter.getResultAt(position).forEach(
					new Command<LibraryBook>() {

						public void execute(LibraryBook libraryBook) {
							showBookDetails(libraryBook);
						};
					});
		} else {
	...
	private void showBookDetails(final LibraryBook libraryBook) {
		...
		TextView descriptionView = (TextView) layout
		...
		if (libraryBook.getLastRead() != null
				&& !libraryBook.getLastRead().equals(new Date(0))) {
		...
		// builder.setPositiveButton(R.string.read, (dialog, which) ->
		// openBook(libraryBook) );
		builder.setPositiveButton(R.string.read,
				new DialogInterface.OnClickListener() {
	...
	private void startImport(File startFolder, boolean copy) {
		ImportTask importTask = new ImportTask(context, libraryService, this,
				config, copy, false);
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		...
		// refreshView();
		// };
		UiUtils.Action toggleListener = new UiUtils.Action() {
		...
		// );
		onMenuPress(menu, R.id.scan_books).thenDo(new UiUtils.Action() {
		...
		// onMenuPress( menu, R.id.about ).thenDo(
		// dialogFactory.buildAboutDialog()::show );
		onMenuPress(menu, R.id.about).thenDo(new UiUtils.Action() {
		...
		// onMenuPress( menu, R.id.profile_day ).thenDo(() ->
		// switchToColourProfile(ColourProfile.DAY) );
		onMenuPress(menu, R.id.profile_day).thenDo(new UiUtils.Action() {
		...
		// onMenuPress( menu, R.id.profile_night ).thenDo(() ->
		// switchToColourProfile(ColourProfile.NIGHT) );
		onMenuPress(menu, R.id.profile_night).thenDo(new UiUtils.Action() {
		...
		if (searchMenuItem != null) {
			...
			if (searchView != null) {
				...
				// searchView.setOnQueryTextListener( UiUtils.onQuery(
				// this::performSearch ));
				searchView.setOnQueryTextListener(UiUtils
						.onQuery(new UiUtils.Operation<String>() {

							@Override
							public void thenDo(String query) {
								performSearch(query);
							}
						}));
				// searchMenuItem.setOnActionExpandListener( onCollapse(() ->
				// performSearch("")));
				searchMenuItem.setOnActionExpandListener(UiUtils
						.onCollapse(new UiUtils.Action() {
				...
				// dialogFactory.showSearchDialog(R.string.search_library,
						...
						.setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {
							...
							public boolean onMenuItemClick(MenuItem item) {
								dialogFactory.showSearchDialog(
										...
										new DialogFactory.SearchCallBack() {
		...
		if (isIntentAvailable(getActivity(), intent)) {
			// onMenuPress( menu, R.id.open_file ).thenDo(
			// this::launchFileManager );
			onMenuPress(menu, R.id.open_file).thenDo(new UiUtils.Action() {
	...
	private void launchFileManager() {
		...
		// Intent readingIntent = new Intent( getActivity(),
	...
	private void performSearch(String query) {
		if (query != null) {
			setSupportProgressBarIndeterminateVisibility(true);
			this.taskQueue.jumpQueueExecuteTask(new LoadBooksTask(config
					.getLastLibraryQuery(), query));
		}
	}
	...
	public void onPrepareOptionsMenu(Menu menu) {
		...
		menu.findItem(R.id.profile_day).setVisible(
				config.getColourProfile() == ColourProfile.NIGHT);
		menu.findItem(R.id.profile_night).setVisible(
				config.getColourProfile() == ColourProfile.DAY);
	}
	...
	private void showImportDialog() {
		...
		final RadioButton scanSpecific = (RadioButton) layout
				.findViewById(R.id.radioScanFolder);
		final TextView folder = (TextView) layout
				.findViewById(R.id.folderToScan);
		final CheckBox copyToLibrary = (CheckBox) layout
				.findViewById(R.id.copyToLib);
		final Button browseButton = (Button) layout
				.findViewById(R.id.browseButton);

		final Option<File> storageBase = config.getStorageBase();
				...
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						if (scanSpecific.isChecked()) {
							String path = folder.getText().toString();
							folderToScan = new File(path);
							config.setScanFolder(path); /*
														 * update custom path
														 * only if used
														 */
						} else {
							File default_storage = storageBase.unsafeGet();
							folderToScan = new File(default_storage
									.getAbsolutePath());
						}
	...
	@Override
	public void importCancelled(int booksImported, List<String> failures,
			boolean emptyLibrary, boolean silent) {
	...
	private void afterImport(int booksImported, List<String> errors,
			boolean emptyLibrary, boolean silent, boolean cancelledByUser) {
		...
		if (booksImported > 0) {
			...
			if (getActivity().getActionBar().getSelectedNavigationIndex() == LibrarySelection.LAST_ADDED
					.ordinal()) {
				loadView(LibrarySelection.LAST_ADDED, "importComplete()");
			} else {
				getActivity().getActionBar().setSelectedNavigationItem(
						LibrarySelection.LAST_ADDED.ordinal());
			}
				...
				builder.setPositiveButton(android.R.string.yes,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
									int which) {
								((PageTurnerActivity) getActivity())
										.launchActivity(CatalogActivity.class);
							}
						});
	...
	public void onAlphabetBarClick(KeyedQueryResult<LibraryBook> result,
			final Character c) {
		...
		// }
		// });
		result.getOffsetFor(toUpperCase(c)).forEach(new Command<Integer>() {
			...
			public void execute(Integer index) {
				...
				if (config.getLibraryView() == LibraryView.BOOKCASE) {
					bookCaseView.setSelection(index);
				} else {
	...
	private class BookListAdapter extends KeyedResultAdapter {
		...
		@Override
		public View getView(int index, final LibraryBook book,
			...
			TextView dateView = (TextView) rowView
					.findViewById(R.id.addedToLibrary);
			TextView progressView = (TextView) rowView
					.findViewById(R.id.readingProgress);
	...
	private class CoverScrollListener implements AbsListView.OnScrollListener {
		...
		public CoverScrollListener() {
			try {
				this.holoDrawable = getResources().getDrawable(
		...
		public void onScroll(AbsListView view, final int firstVisibleItem,
			...
			// if (alphabetAdapter != null &&
			// !keyChar.equals(alphabetAdapter.getHighlightChar())) {
			...
			//
			// LOG.debug( "Loading items " + firstVisibleItem + " to " +
			// lastVisibleItem + " of " + totalItemCount );
			//
			// for ( CoverCallback callback: localList ) {
			// if ( callback.viewIndex >= firstVisibleItem && callback.viewIndex
			// <= lastVisibleItem ) {
			// callback.run();
			// }
			// }
			...
			this.lastRunnable = new Runnable() {
				...
				public void run() {
					if (bookAdapter.isKeyed()) {
						...
						if (key.length() > 0) {
							...
							if (keyChar.equals(lastCharacter)) {
								...
								lastCharacter = keyChar;
								List<Character> alphabet = bookAdapter
										.getAlphabet();

								// If the highlight-char is already set, this
								// means the
								// user clicked the bar, so don't scroll it.
								if (alphabetAdapter != null
										&& !keyChar.equals(alphabetAdapter
												.getHighlightChar())) {
									alphabetAdapter.setHighlightChar(keyChar);
									alphabetBar.setSelection(alphabet
											.indexOf(keyChar));
								}
					...
					for (CoverCallback callback : localList) {
						if (callback.viewIndex >= firstVisibleItem
								&& callback.viewIndex <= lastVisibleItem) {
							callback.run();
						}
					}
				}
	...
	private class CoverCallback {
		...
		public void run() {
			try {
				...
				getCover(book).forEach(new Command<Drawable>() {
	...
	private class BookCaseAdapter extends KeyedResultAdapter {
		...
		@Override
		public View getView(final int index, final LibraryBook object,
				View convertView, ViewGroup parent) {

			View result;

			if (convertView == null) {
				LayoutInflater inflater = PlatformUtil
			...
			// LibraryFragment.this.onItemLongClick(null, null, index, 0));
			...
			final ImageView image = (ImageView) result
					.findViewById(R.id.bookCover);
	...
	private class AlphabetAdapter extends ArrayAdapter<Character> {
		...
		public AlphabetAdapter(Context context, int layout, int view,
	...
	private void loadQueryData(QueryResult<LibraryBook> result) {
		...
		if (result instanceof KeyedQueryResult
				&& result.getSize() >= ALPHABET_THRESHOLD) {
			...
			alphabetAdapter = new AlphabetAdapter(getActivity(),
					R.layout.alphabet_line, R.id.alphabetLabel,
					keyedResult.getAlphabet());

			alphabetBar.setAdapter(alphabetAdapter);

			// alphabetBar.setOnItemClickListener( (a, b, index, c) ->
			// onAlphabetBarClick(keyedResult,
			// keyedResult.getAlphabet().get(index) ));
			alphabetBar
					.setOnItemClickListener(new AdapterView.OnItemClickListener() {
						...
						public void onItemClick(AdapterView<?> parent,
								View view, int position, long id) {
							onAlphabetBarClick(keyedResult, keyedResult
									.getAlphabet().get(position));
						}
					});
	...
	private class LoadBooksTask extends
			QueueableAsyncTask<String, Integer, QueryResult<LibraryBook>> {
		...
		public void doOnPostExecute(Option<QueryResult<LibraryBook>> result) {
			...
			//
			// if (filter == null && sel ==
			// Configuration.LibrarySelection.LAST_ADDED && r.getSize() == 0 &&
			// !askedUserToImport) {
			...
			result.match(new OptionMatcher<QueryResult<LibraryBook>>() {
				...
				@Override
				public void caseSome(QueryResult<LibraryBook> r) {
					loadQueryData(r);

					if (filter == null
							&& sel == Configuration.LibrarySelection.LAST_ADDED
							&& r.getSize() == 0 && !askedUserToImport) {
						askedUserToImport = true;
						buildImportQuestionDialog();
						importQuestion.show();
					}
