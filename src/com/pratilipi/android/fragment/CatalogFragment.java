...
import com.pratilipi.android.catalog.Catalog;
import com.pratilipi.android.catalog.CatalogListAdapter;
import com.pratilipi.android.catalog.CatalogParent;
import com.pratilipi.android.catalog.LoadFeedCallback;
import com.pratilipi.android.catalog.LoadOPDSTask;
import com.pratilipi.android.catalog.LoadThumbnailTask;
import com.pratilipi.android.catalog.ParseBinDataTask;
...
public class CatalogFragment extends Fragment implements LoadFeedCallback {
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		adapter.setImageLoader(new CatalogListAdapter.CatalogImageLoader() {

			@Override
			public Option<Drawable> getThumbnailFor(String baseURL, Link link) {
				return option(thumbnailCache.get(link.getHref()));
			}
		});
		...
		catalogList
				.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						Option<Entry> entry = adapter.getItem(position);
						entry.forEach(new Command<Entry>() {

							@Override
							public void execute(Entry e) {
								onEntryClicked(e, position);
							}
						});
					}
				});
	...
	public void onBecameVisible() {
		...
		// );
		adapter.getFeed().forEach(new Command<Feed>() {

			public void execute(Feed f) {
				((CatalogParent) getActivity()).onFeedLoaded(f);
			};
		});
	}
	...
	public void performSearch(String searchTerm) {
		if (searchTerm != null && searchTerm.length() > 0) {
			final String searchString = URLEncoder.encode(searchTerm);
			...
			// }));
			feed.forEach(new Command<Feed>() {

				@Override
				public void execute(Feed f) {
					f.getSearchLink().forEach(new Command<Link>() {

						@Override
						public void execute(Link searchLink) {
							String linkUrl = searchLink.getHref();
							linkUrl = linkUrl.replace(
									AtomConstants.SEARCH_TERMS, searchString);
							loadURL(null, linkUrl, false, true,
									ResultType.REPLACE);
						}
					});
				}
	...
	private void replaceFeed(Entry entry, String href, boolean asDetailsFeed) {
		...
		((CatalogParent) getActivity()).loadFeed(entry, href, baseURL,
				asDetailsFeed);
	}
	...
	public void loadURL(Entry entry, String url, boolean asDetailsFeed,
			boolean asSearchFeed, ResultType resultType) {
	...
	public void onPrepareOptionsMenu(Menu menu) {
		...
		// boolean searchEnabled = !isEmpty( (Option<Link>)
		// feed.flatMap(Feed::getSearchLink) );
		boolean searchEnabled = !isEmpty(feed
				.flatMap(new Functor<Feed, Option<Link>>() {

					@Override
					public Option<Link> execute(Feed f) {
						f.getSearchLink();
						return null;
					}
				}));
	...
	private class LoadingScrollListener implements AbsListView.OnScrollListener {
		...
		private void loadNextFeed(final int firstVisibleItem,
			...
			if (totalItemCount - lastVisibleItem <= LOAD_THRESHOLD
				...
				Option<Entry> lastEntry = adapter
						.getItem(adapter.getCount() - 1);

				// lastEntry.flatMap(Entry::getFeed).forEach( feed ->
				// feed.getNextLink().forEach( link -> {
				// if (! link.getHref().equals(lastLoadedUrl)) {
				// Entry nextEntry = new Entry();
				// nextEntry.setFeed(feed);
				// nextEntry.addLink(link);
				//
				// LOG.debug("Starting download for " + link.getHref() +
				// " after scroll");
				//
				// lastLoadedUrl = link.getHref();
				// loadURL(nextEntry, link.getHref(), false, false,
				// ResultType.APPEND);
				// }
				// }));
				lastEntry.flatMap(new Functor<Entry, Option<Feed>>() {

					public Option<Feed> execute(Entry e) {
						return e.getFeed();
					};
				}).forEach(new Command<Feed>() {

					@Override
					public void execute(final Feed feed) {
						feed.getNextLink().forEach(new Command<Link>() {
							...
							public void execute(Link link) {
								if (!link.getHref().equals(lastLoadedUrl)) {
									...
									lastLoadedUrl = link.getHref();
									loadURL(nextEntry, link.getHref(), false,
											false, ResultType.APPEND);
								}
							}
						});
					}
		...
		private void loadThumbnails(final int firstVisibleItem,
			...
			updater = new Runnable() {
				...
				public void run() {
					for (int i = 0; i < visibleItemCount; i++) {
						Option<Entry> entry = adapter.getItem(firstVisibleItem
								+ i);
						...
						entry.forEach(new Command<Entry>() {
							...
							public void execute(final Entry e) {
								e.getFeed().forEach(new Command<Feed>() {

									public void execute(Feed feed) {
										Catalog.getImageLink(feed, e).forEach(
												new Command<Link>() {
