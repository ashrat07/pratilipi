...
import com.pratilipi.android.catalog.DownloadFileTask;
 ...
 */
// public class BookDetailsFragment extends RoboSherlockFragment implements
// LoadFeedCallback {
public class BookDetailsFragment extends Fragment implements LoadFeedCallback {
	...
	private void doSetFeed(Feed feed) {
		...
		if (!isEmpty(epubLink)) {
			...
			try {
				final URL url = new URL(new URL(base), epubLink.unsafeGet()
						.getHref());

				// downloadButton.setOnClickListener( v -> startDownload(true,
				// url.toExternalForm() ));
		...
		final Option<Link> buyLink = entry.getBuyLink();
		...
		final LoadThumbnailTask task = this.loadThumbnailTaskProvider.get();
		task.setLoadFeedCallback(this);
		task.setBaseUrl(feed.getURL());

		// imgLink.forEach( task::execute );
		imgLink.forEach(new Command<Link>() {
	...
	public void startDownload(final boolean openOnCompletion, final String url) {
		...
		if (!openOnCompletion
			...
			task.setCallBack(new NotificationBarCallback(getActivity()
					.getBaseContext(), title, openOnCompletion));
		} else {
			task.setCallBack(new ProgressDialogCallback(getActivity()
					.getBaseContext(), task, openOnCompletion));
		}
	...
	private abstract class AbstractDownloadCallback implements
			DownloadFileTask.DownloadFileCallback {

		private boolean openOnCompletion;
		private Context context;

		public AbstractDownloadCallback(Context context,
	...
	private class NotificationBarCallback extends AbstractDownloadCallback {
		...
		public void onDownloadStart() {
			...
			builder.setContentTitle(title).setContentText(downloadSubtitle)
	...
	private class ProgressDialogCallback extends AbstractDownloadCallback
			implements DialogInterface.OnCancelListener {

		private ProgressDialog downloadDialog;
		private DownloadFileTask task;

		private ProgressDialogCallback(Context context, DownloadFileTask task,
				boolean openOnCompletion) {
