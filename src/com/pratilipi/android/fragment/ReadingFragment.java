...
package com.pratilipi.android.fragment;
...
import com.pratilipi.android.activity.LibraryActivity;
import com.pratilipi.android.activity.MediaButtonReceiver;
import com.pratilipi.android.activity.ReadingActivity;
import com.pratilipi.android.animation.Animations;
import com.pratilipi.android.animation.Animator;
import com.pratilipi.android.animation.PageCurlAnimator;
import com.pratilipi.android.animation.PageTimer;
import com.pratilipi.android.animation.RollingBlindAnimator;
import com.pratilipi.android.bookmark.Bookmark;
import com.pratilipi.android.bookmark.BookmarkDatabaseHelper;
import com.pratilipi.android.dto.HighLight;
import com.pratilipi.android.dto.SearchResult;
import com.pratilipi.android.dto.TocEntry;
import com.pratilipi.android.epub.SearchTextTask;
import com.pratilipi.android.htmlspanner.spans.CenterSpan;
import com.pratilipi.android.library.LibraryService;
import com.pratilipi.android.sync.AccessException;
import com.pratilipi.android.sync.BookProgress;
import com.pratilipi.android.sync.ProgressService;
import com.pratilipi.android.tts.SpeechCompletedCallback;
import com.pratilipi.android.tts.TTSPlaybackItem;
import com.pratilipi.android.tts.TTSPlaybackQueue;
import com.pratilipi.android.util.ActionModeBuilder;
import com.pratilipi.android.util.Configuration;
import com.pratilipi.android.util.Configuration.AnimationStyle;
import com.pratilipi.android.util.Configuration.ColourProfile;
import com.pratilipi.android.util.Configuration.ReadingDirection;
import com.pratilipi.android.util.Configuration.ScrollStyle;
import com.pratilipi.android.util.DialogFactory;
import com.pratilipi.android.util.TextUtil;
import com.pratilipi.android.util.UiUtils;
import com.pratilipi.android.view.AnimatedImageView;
import com.pratilipi.android.view.HighlightManager;
import com.pratilipi.android.view.NavGestureDetector;
import com.pratilipi.android.view.NavigationCallback;
import com.pratilipi.android.view.ProgressListAdapter;
import com.pratilipi.android.view.SearchResultAdapter;
import com.pratilipi.android.view.bookview.BookView;
import com.pratilipi.android.view.bookview.BookViewListener;
import com.pratilipi.android.view.bookview.SelectedWord;
import com.pratilipi.android.view.bookview.TextLoader;
import com.pratilipi.android.view.bookview.TextSelectionCallback;
import com.pratilipi.android.yuku.ambilwarna.AmbilWarnaDialog;
...
import static com.pratilipi.android.util.PlatformUtil.*;
import static com.pratilipi.android.util.UiUtils.onMenuPress;

public class ReadingFragment extends Fragment implements BookViewListener,
	...
	/*
	 * This is actually a RemoteControlClient, but we declare it as an object,
	 * since the RemoteControlClient class is only available in ICS and later.
	 */
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB
			...
			return inflater.inflate(R.layout.fragment_reading_fs, container,
					false);
		} else {
			return inflater
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (ttsIsRunning()) {
			...
			// this.ttsPlaybackItemQueue.updateSpeechCompletedCallbacks(
			// this::speechCompleted );
			this.ttsPlaybackItemQueue
					.updateSpeechCompletedCallbacks(new SpeechCompletedCallback() {

						@Override
						public void speechCompleted(TTSPlaybackItem item,
								MediaPlayer mediaPlayer) {
							speechCompleted(item, mediaPlayer);
						}
					});
			uiHandler.post(progressBarUpdater);
		}
		...
		// -> {
		//
		// LOG.debug("Detected change of visibility in action-bar: visible=" +
		// isVisible );
		// int visibility = isVisible ? View.VISIBLE : View.GONE;
		// titleBarLayout.setVisibility( visibility );
		//
		// });
		activity.getActionBar().addOnMenuVisibilityListener(
	...
	public void saveConfigState() {
		...
		savedConfigState.sansSerifFontName = config.getSansSerifFontFamily()
				.getName();
	...
	private void playBeep(boolean error) {
		...
		try {
			...
			beepPlayer.setDataSource(descriptor.getFileDescriptor(),
	...
	private void startTextToSpeech() {
		...
		if (isEmpty(fosOption)) {
			// Could not get base folder for TTS
	...
	/**
	 * Splits the text to be spoken into chunks and streams them to disk. This
	 * method should NOT be called on the UI thread!
	 */
	private void doStreamTTSToDisk() {
		...
		String textToSpeak = text.map(new Functor<Spanned, String>() {
			...
			public String execute(Spanned c) {
		...
		try {
			...
			for (int i = 0; i < parts.size() && ttsIsRunning(); i++) {
				...
				try {
					File pageFile = new File(ttsFolder, "tts_"
				...
				streamPartToDisk(pageName, part, offset, textToSpeak.length(),
						lastPart);
	...
	private void streamPartToDisk(String fileName, String part, int offset,
			int totalLength, boolean endOfPage) throws TTSFailedException {
		...
		if (part.trim().length() > 0 || endOfPage) {
			...
			TTSPlaybackItem item = new TTSPlaybackItem(part, new MediaPlayer(),
					totalLength, offset, endOfPage, fileName);
			...
			// Skipping part, since it's empty.
		}
	}
	...
	private void showTTSFailed(final String message) {
		...
		// textBuilder.append("\n").append(message);
		//
		// Toast.makeText(context, textBuilder.toString(),
	...
	public void onStreamingCompleted(final String wavFile) {
		...
		if (!ttsItemPrep.containsKey(wavFile)) {
			// Got onStreamingCompleted for but there is no corresponding
			// TTSPlaybackItem!
			return;
		}
				...
				if (item != null) {
					...
					if (mediaPlayer != null && mediaPlayer.isPlaying()) {
						...
						if (phoneState == TelephonyManager.CALL_STATE_RINGING
							...
							double percentage = (double) mediaPlayer
									.getCurrentPosition()
									/ (double) mediaPlayer.getDuration();

							mediaProgressBar.setMax(mediaPlayer.getDuration());
							mediaProgressBar.setProgress(mediaPlayer
									.getCurrentPosition());

							int currentDuration = item.getOffset()
									+ (int) (percentage * item.getText()
											.length());

							bookView.navigateTo(bookView.getIndex(),
									currentDuration);
							...
							&& pausedBecauseOfCall) {
						// We reset to the start of the current section before
						// resuming playback.
	...
	private void subscribeToMediaButtons() {
		if (this.mediaReceiver == null) {
			...
			IntentFilter filter = new IntentFilter(
					MediaButtonReceiver.INTENT_PAGETURNER_MEDIA);
			context.registerReceiver(mediaReceiver, filter);

			ComponentName remoteControlsReceiver = new ComponentName(context,
					MediaButtonReceiver.class);
			audioManager
					.registerMediaButtonEventReceiver(remoteControlsReceiver);
	...
	private void startPlayback() {
		...
		if (this.remoteControlClient != null) {
			...
			// Focus: remoteControlClient was null
		}
	...
	private void setMetaData() {
		...
		RemoteControlClient.MetadataEditor editor = localRemoteControlClient
				.editMetadata(true);
	...
	private void displayPageNumber(int pageNumber) {
		...
		pageNumberView.setTypeface(config.getDefaultFontFamily()
				.getDefaultTypeface());
	...
	private void updateFromPrefs() {
		...
		if (config.isFullScreenEnabled() != savedConfigState.fullscreen
				...
				|| !config.getDefaultFontFamily().getName()
						.equalsIgnoreCase(savedConfigState.fontName)
				|| !config.getSerifFontFamily().getName()
						.equalsIgnoreCase(savedConfigState.serifFontName)
				|| !config.getSansSerifFontFamily().getName()
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		if (this.selectedWord != null) {
			...
			if (isDictionaryAvailable()) {
				...
				// onMenuPress(item).thenDo( () ->
				// lookupDictionary(word.toString()) );
				onMenuPress(item).thenDo(new UiUtils.Action() {
			...
			onMenuPress(lookUpWikipediaItem).thenDo(new UiUtils.Action() {
	...
	public void highLight(int from, int to, String selectedText) {
		...
		this.highlightManager.registerHighlight(fileName, text,
				bookView.getIndex(), pageStart + from, pageStart + to);
	...
	private void showHighlightEditDialog(final HighLight highLight) {
		...
		// });
		editalert.setPositiveButton(R.string.save_note,
		...
		// -> {} );
		editalert.setNegativeButton(android.R.string.cancel,
				new DialogInterface.OnClickListener() {
		...
		// highlightManager.saveHighLights();
		// });
		editalert.setNeutralButton(R.string.clear_note,
				new DialogInterface.OnClickListener() {
	...
	private void showHighlightColourDialog(final HighLight highLight) {
		...
		AmbilWarnaDialog ambilWarnaDialog = new AmbilWarnaDialog(context,
				highLight.getColor(),
	...
	private void onBookmarkLongClick(final Bookmark bookmark) {
		...
		actionModeBuilderProvider
				.get()
				.setTitle(R.string.bookmark_options)
				.setOnCreateAction(
						new ActionModeBuilder.ActionModeAction<Menu>() {

							@Override
							public boolean perform(ActionMode actionMode,
									Menu item) {
								MenuItem delete = item.add(R.string.delete);
								delete.setIcon(R.drawable.trash_can);

								return true;
							}
						})
				.setOnActionItemClickedAction(
						new ActionModeBuilder.ActionModeAction<MenuItem>() {
	...
	public void onHighLightClick(final HighLight highLight) {
		...
		final Map<String, Command<HighLight>> commands = new HashMap<>();
		// commands.put( getString(R.string.edit), this::showHighlightEditDialog
		// );
		commands.put(getString(R.string.edit), new Command<HighLight>() {

			public void execute(HighLight highLight) {
				showHighlightEditDialog(highLight);
			};
		});
		// commands.put( getString(R.string.delete), this::deleteHightlight );
		commands.put(getString(R.string.delete), new Command<HighLight>() {
		...
		// commands.put( getString(R.string.set_colour),
		// this::showHighlightColourDialog );
		commands.put(getString(R.string.set_colour), new Command<HighLight>() {
		...
		actionModeBuilderProvider
				...
				.setOnCreateAction(
						new ActionModeBuilder.ActionModeAction<Menu>() {
							...
							public boolean perform(ActionMode actionMode,
									Menu item) {
								item.add(R.string.edit)
										.setIcon(R.drawable.edit);
								item.add(R.string.set_colour).setIcon(
										R.drawable.color);
								item.add(R.string.delete).setIcon(
				...
				.setOnActionItemClickedAction(
						new ActionModeBuilder.ActionModeAction<MenuItem>() {
							...
							public boolean perform(ActionMode actionMode,
									MenuItem item) {
								Command<HighLight> cmd = commands.get(item
										.getTitle());
	...
	private void deleteHightlight(final HighLight highLight) {
		...
		if (highLight.getTextNote() != null
				&& highLight.getTextNote().length() > 0) {
			...
			// Toast.makeText( context,R.string.highlight_deleted,
			// Toast.LENGTH_SHORT ).show();
			// bookView.update();
			// })
			// .show();
	...
	private String getLanguageCode() {
		if (this.language == null || this.language.equals("")
				|| this.language.equalsIgnoreCase("und")) {
	...
	public void lookupWikipedia(String text) {
		...
		openBrowser("http://" + getLanguageCode()
	...
	public void lookupWiktionary(String text) {
		openBrowser("http://" + getLanguageCode()
				+ ".wiktionary.org/w/index.php?title=Special%3ASearch&search="
				+ URLEncoder.encode(text));
	...
	public void errorOnBookOpening(String errorMessage) {
		...
		if (readingActivity != null) {
			...
			builder.setContentTitle(getString(R.string.app_name))
					...
					.setSmallIcon(R.drawable.cross).setAutoCancel(true);
			...
			notificationManager
					.notify(errorMessage.hashCode(), builder.build());
	...
	public void parseEntryComplete(String name) {
		...
		if (activity != null) {
			...
			if (this.ttsPlaybackItemQueue.isActive()
					&& this.ttsPlaybackItemQueue.isEmpty()) {
				streamTTSToDisk();
			}
	...
	public boolean dispatchMediaKeyEvent(KeyEvent event) {
		...
		case KeyEvent.KEYCODE_MEDIA_PAUSE:
			return simulateButtonPress(action, R.id.playPauseButton,
					playPauseButton);
	...
	private boolean simulateButtonPress(int action, int idToSend,
			ImageButton buttonToClick) {
	...
	public boolean dispatchKeyEvent(KeyEvent event) {
		...
		if (searchMenuItem != null && searchMenuItem.isActionViewExpanded()) {
			boolean result = searchMenuItem.getActionView().dispatchKeyEvent(
					event);
	...
	private void doAutoScroll() {
		...
		if (dummyView.getAnimator() == null) {
			// BookView no longer has an animator. Aborting rolling blind
			stopAnimating();
		} else {
			...
			if (anim.isFinished()) {
				...
				// uiHandler.postDelayed(this::doAutoScroll,
	...
	private void preparePageTimer() {
		...
		after.forEach(new Command<Bitmap>() {
			...
			@Override
			public void execute(Bitmap b) {
				PageTimer timer = new PageTimer(b, pageNumberView.getHeight());
				timer.setSpeed(config.getScrollSpeed());
				dummyView.setAnimator(timer);
			}
		});
	 ...
	 * 
	 * This method advances the animator by 1 frame, and then places itself back
	 * on the background queue, passing along the same animator.
	 * 
	...
	private void prepareSlide(Animation inAnim, Animation outAnim) {
		...
		/*
		 * TODO: is this OK? We don't set anything when we get None instead of
		 * Some.
		 */
	...
	public void share(int from, int to, String selectedText) {
		...
		if (pageNumber != -1) {
			text = text
					+ String.format(getString(R.string.page_number_of),
	...
	public boolean onLeftEdgeSlide(int value) {
		...
		if (config.isBrightnessControlEnabled() && value != 0) {
			...
			if (brightnessToast == null) {
				brightnessToast = Toast.makeText(context, brightness + ": "
	...
	@Override
	public void onWordLongPressed(int startOffset, int endOffset,
			CharSequence word) {
	...
	private void showPickProgressDialog(final List<BookProgress> results) {
		...
		ProgressListAdapter adapter = new ProgressListAdapter(context,
				bookView, results);
	...
	public boolean hasTableOfContents() {
		...
		return !isEmpty(toc.getOrElse(new ArrayList<TocEntry>()));
	}

	public List<NavigationCallback> getTableOfContents() {

		List<TocEntry> entries = this.bookView.getTableOfContents().getOrElse(
				new ArrayList<TocEntry>());
		...
		// )
		// );
		return map(entries, new Functor<TocEntry, NavigationCallback>() {

			public NavigationCallback execute(final TocEntry tocEntry) {
				return new NavigationCallback(tocEntry.getTitle(), "",
						new UiUtils.Action() {
	...
	private void sendProgressUpdateToServer(final int index, final int position) {
		...
		backgroundHandler.post(new Runnable() {
			...
			public void run() {
				try {
					progressService.storeProgress(fileName, index, position,
							progressPercentage);
				} catch (Exception e) {
	...
	public void performSearch(String query) {
		...
		final int[] counter = { 0 }; // Yes, this is essentially a pointer to an
										// int :P
		...
		//
		// });
		task.setOnPreExecute(new UiUtils.Action() {
		...
		// }
		//
		// searchProgress.setProgress(bookView.getPercentageFor(res.getIndex(),
		// res.getStart()));
		// }
		// });
		task.setOnProgressUpdate(new Command<SearchResult[]>() {

			@Override
			public void execute(SearchResult[] values) {
				if (isAdded()) {
					...
					searchProgress.setProgress(bookView.getPercentageFor(
							res.getIndex(), res.getStart()));
				}
			}
		});
		...
		// }
		// });
		task.setOnCancelled(new Command<Option<List<SearchResult>>>() {

			@Override
			public void execute(Option<List<SearchResult>> result) {
				if (isAdded()) {
					Toast.makeText(context, R.string.search_cancelled,
							Toast.LENGTH_LONG).show();
				}
		...
		// }
		// });
		task.setOnPostExecute(new Command<Option<List<SearchResult>>>() {
			...
			public void execute(Option<List<SearchResult>> result) {
				...
				if (!task.isCancelled() && isAdded()) {
					...
					List<SearchResult> resultList = result
							.getOrElse(new ArrayList<SearchResult>());
	...
	public void onSearchRequested() {
		if (this.searchMenuItem != null
				&& searchMenuItem.getActionView() != null) {
			getActivity().getActionBar().show();
			this.searchMenuItem.expandActionView();
			this.searchMenuItem.getActionView().requestFocus();
		} else {
			// dialogFactory.showSearchDialog(R.string.search_text,
			// R.string.enter_query, this::performSearch);
			dialogFactory.showSearchDialog(R.string.search_text,
					R.string.enter_query, new DialogFactory.SearchCallBack() {
	...
	private void showSearchResultDialog(final List<SearchResult> results) {
		...
		SearchResultAdapter adapter = new SearchResultAdapter(context,
				bookView, results);
		builder.setAdapter(adapter, adapter);

		AlertDialog dialog = builder.create();
		dialog.setOwnerActivity(getActivity());
		// dialog.setOnDismissListener(d -> isSearchResultsDialogShowing =
		// false);
	...
	public List<NavigationCallback> getSearchResults() {
		...
		for (final SearchResult searchResult : this.searchResults) {
			...
			int percentage = bookView.getPercentageFor(searchResult.getIndex(),
					searchResult.getStart());
			int pageNumber = bookView.getPageNumberFor(searchResult.getIndex(),
					searchResult.getStart());
			...
			if (pageNumber != -1) {
				text = String.format(
						context.getString(R.string.page_number_of), pageNumber,
						totalNumberOfPages)
						+ " (" + percentage + "%)";
			} else {
				text = percentage + "%";
	...
	public boolean hasHighlights() {
		...
		List<HighLight> highLights = this.highlightManager
				.getHighLights(bookView.getFileName());
	...
	public boolean hasBookmarks() {
		...
		List<Bookmark> bookmarks = this.bookmarkDatabaseHelper
				.getBookmarksForFile(bookView.getFileName());
	...
	public List<NavigationCallback> getBookmarks() {
		...
		for (final Bookmark bookmark : bookmarks) {
			...
			// NavigationCallback callback = new NavigationCallback(
			// bookmark.getName(), finalText )
			// .setOnClick( () -> bookView.navigateTo( bookmark.getIndex(),
			// bookmark.getPosition() ))
			// .setOnLongClick( () -> onBookmarkLongClick(bookmark) );
			NavigationCallback callback = new NavigationCallback(
					bookmark.getName(), finalText).setOnClick(
					new UiUtils.Action() {

						@Override
						public void perform() {
							bookView.navigateTo(bookmark.getIndex(),
									bookmark.getPosition());
						}
					}).setOnLongClick(new UiUtils.Action() {
	...
	public List<NavigationCallback> getHighlights() {
		...
		for (final HighLight highLight : highLights) {
			...
			final String finalText = getHighlightLabel(highLight.getIndex(),
					highLight.getStart(), highLight.getTextNote());

			// NavigationCallback callback = new NavigationCallback(
			// highLight.getDisplayText(), finalText )
			// .setOnClick( () -> bookView.navigateTo( highLight.getIndex(),
			// highLight.getStart() ))
			// .setOnLongClick( () -> onHighLightClick(highLight) );
			NavigationCallback callback = new NavigationCallback(
					highLight.getDisplayText(), finalText).setOnClick(
					new UiUtils.Action() {

						@Override
						public void perform() {
							bookView.navigateTo(highLight.getIndex(),
									highLight.getStart());
						}
					}).setOnLongClick(new UiUtils.Action() {
	...
	private class ManualProgressSync extends
		...
		protected void onPostExecute(Option<List<BookProgress>> progress) {
			...
			if (isEmpty(progress)) {
				...
				// alertDialog.setNeutralButton(android.R.string.ok, (dialog,
				...
				if (!actualProgress.isEmpty()) {
					...
					Toast.makeText(context, R.string.no_sync_points,
							Toast.LENGTH_LONG).show();
				}
			}
		}
	...
	private class DownloadProgressTask extends
		...
		protected void onPreExecute() {
			if (isAdded()) {
				final ProgressDialog progressDialog = getWaitDialog();
				progressDialog.setMessage(getString(R.string.syncing));
				progressDialog.setCancelable(true);
				// progressDialog.setOnCancelListener( d -> {
				...
				progressDialog
						.setOnCancelListener(new DialogInterface.OnCancelListener() {
							...
							public void onCancel(DialogInterface dialog) {
								DownloadProgressTask.this.cancel(true);
								progressDialog
										.setMessage(getString(R.string.cancelling));
								progressDialog.show();
							}
						});
		...
		protected Option<BookProgress> doInBackground(None... params) {
			try {
				...
				return firstOption(updates
						.getOrElse(new ArrayList<BookProgress>()));
		...
		protected void onPostExecute(Option<BookProgress> progress) {
			...
			// }
			// });
			progress.forEach(new Command<BookProgress>() {

				@Override
				public void execute(BookProgress p) {
					int index = bookView.getIndex();
					int pos = bookView.getProgressPosition();
	...
	private class PageTurnerMediaReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(
					MediaButtonReceiver.INTENT_PAGETURNER_MEDIA)) {
				KeyEvent event = new KeyEvent(intent.getIntExtra("action", 0),
