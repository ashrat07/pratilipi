...
import com.pratilipi.android.library.SqlLiteLibraryService;
import com.pratilipi.android.scheduling.TaskQueue;
import com.pratilipi.android.ssl.EasySSLSocketFactory;
import com.pratilipi.android.sync.PageTurnerWebProgressService;
 ...
 * 
 * This module determines the implementations used to inject dependencies used
 * in actually running the app.
 * 
 * @author Alex Kuiper
 * 
 */
public class PageTurnerModule extends AbstractModule {
	...
	protected void configure() {
		...
		bind(ProgressService.class).to(PageTurnerWebProgressService.class).in(
				Singleton.class);
			...
			if (site.getUserName() != null && site.getUserName().length() > 0) {
				try {
					...
					client.getCredentialsProvider().setCredentials(
							...
							new UsernamePasswordCredentials(site.getUserName(),
									site.getPassword()));
