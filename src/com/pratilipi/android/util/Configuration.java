...
import com.pratilipi.android.dto.PageOffsets;
...
public class Configuration {
	...
	public Configuration(Context context) {
		...
		if (IS_NOOK_TOUCH
				&& this.settings.getString(KEY_DEVICE_NAME, null) == null) {
			...
			final String no_animation = AnimationStyle.NONE.name().toLowerCase(
					Locale.US);
	...
	public Class<? extends PageTurnerActivity> getLastActivity() {
		...
		try {
			return (Class<? extends PageTurnerActivity>) Class
					.forName(lastActivityString);
	...
	public void setLastActivity(
			Class<? extends PageTurnerActivity> activityClass) {
	...
	public Option<List<List<Integer>>> getPageOffsets(String fileName) {
		...
		try {
			...
			// Could not retrieve page offsets
			return none();
		}
	}
	...
	public List<HighLight> getHightLights(String fileName) {
		...
		if (prefs.contains(KEY_HIGHLIGHTS)) {
			return HighLight.fromJSON(fileName,
					prefs.getString(KEY_HIGHLIGHTS, "[]"));
		}
	...
	public List<CustomOPDSSite> getCustomOPDSSites() {
		...
		try {
			...
			// Could not retrieve custom opds sites
		}
	...
	public boolean isDimSystemUI() {
		return isFullScreenEnabled()
				&& settings.getBoolean(KEY_DIM_SYSTEM_UI, false);
	}
	...
	public boolean isUseColoursFromCSS() {
		...
		if (getColourProfile() == ColourProfile.NIGHT) {
			return settings.getBoolean(PREFIX_NIGHT + "_" + setting,
					nightDefault);
		} else {
	...
	public Option<File> getDownloadsFolder() {
		...
		return firstOption(asList(ContextCompat.getExternalFilesDirs(context,
				"Downloads")));
	}
	...
	public Option<File> getLibraryFolder() {
		...
		// baseFolder -> new File(baseFolder.getAbsolutePath() +
		// "/PageTurner/Books") );
		Option<File> libraryFolder = getStorageBase().map(
				new Functor<File, File>() {

					@Override
					public File execute(File baseFolder) {
	...
	public void setCopyToLibraryOnScan(boolean value) {
		settings.edit().putBoolean(KEY_COPY_TO_LIB, value).commit();
	}
	...
	public void setUseCustomScanFolder(boolean value) {
		settings.edit().putBoolean(KEY_USE_SCAN_FOLDER, value).commit();
	}

	/**
	 * Return the default folder path which is shown for the "scan for books"
	 * custom directory
	 */
	private String getDefaultScanFolder() {
		return Configuration.IS_NOOK_TOUCH ? "/media" : /*
														 * Nook's default
														 * internal content
														 * storage (accessible
														 * via USB) is under
	...
	/**
	 * Set the folder path to show for "scan for books" custom directory. Will
	 * only save a setting if the default actually changed.
	 */
	...
	/*
	 * Returns the available bitmap memory. On newer Android versions this is
	 * the same as the normaL heap memory.
	 */
