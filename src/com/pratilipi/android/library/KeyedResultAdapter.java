 ...
 * 
 */
public abstract class KeyedResultAdapter extends
		QueryResultAdapter<LibraryBook> implements SectionIndexer {
	...
	public int getSectionForPosition(int position) {
		...
		Option<Character> characterFor = this.keyedResult
				.getCharacterFor(position);

		// return characterFor.match(
		// c -> this.keyedResult.getAlphabet().indexOf(c),
		// () -> 0
		// );
		characterFor.match(new OptionMatcher<Character>() {

			public void caseSome(Character c) {
				section = keyedResult.getAlphabet().indexOf(c);
			};
