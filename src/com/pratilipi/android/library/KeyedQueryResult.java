...
/**
 * Special QueryResult which buffers the keys, allowing for direct access.
 * 
 * @author Alex Kuiper
 * 
 * @param <T>
...
public abstract class KeyedQueryResult<T> extends QueryResult<T> {
	...
	private List<Character> calculateAlphaBet() {
		...
		// )
		// );
		SortedSet<Character> firstLetters = new TreeSet<>(collect(
				select(keys, new Filter<String>() {

					@Override
					public Boolean execute(String k) {
						return k.length() > 0;
					}
				}), new Functor<String, Character>() {

					@Override
					public Character execute(String key) {
						return key.charAt(0);
					}
				}));
