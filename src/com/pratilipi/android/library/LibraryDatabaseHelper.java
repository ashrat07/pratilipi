...
public class LibraryDatabaseHelper extends SQLiteOpenHelper {
	...
	public enum Field {
		file_name("text primary key"), title("text"), a_first_name("text"), a_last_name(
				"text"), date_added("integer"), date_last_read("integer"), description(
				"text"), cover_image("blob"), progress("integer");
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		if (oldVersion == 3) {
			db.execSQL("ALTER TABLE " + LIB_BOOKS_TABLE
	...
	public boolean hasBook(String fileName) {
		...
		Cursor findBook = getDataBase().query(LIB_BOOKS_TABLE,
				fieldsAsString(fields), whereClause, args, null, null, null);
	...
	private static String getFilterClause(String filter, String existingClause) {
		...
		String whereClause = "(" + Field.a_first_name + " like ? " + " or "
				+ Field.a_last_name + " like ? " + " or " + Field.title
				+ " like ? )";
	...
	public synchronized KeyedQueryResult<LibraryBook> findByField(
			Field fieldName, String fieldValue, Field orderField,
			Order ordering, String filter) {
		...
		Cursor cursor = getDataBase().query(LIB_BOOKS_TABLE,
				fieldsAsString(Field.values()), whereClause, args, null, null,
	...
	public synchronized QueryResult<LibraryBook> findAllOrderedBy(
			Field fieldName, Order order, String filter) {

		String whereClause = fieldName != null ? fieldName.toString()
				+ " is not null" : null;
	...
	private List<String> getKeys(Field fieldName, Order order) {
		...
		Cursor fieldCursor = getDataBase().query(
				LIB_BOOKS_TABLE,
				keyField,
				null,
				new String[0],
	...
	public synchronized KeyedQueryResult<LibraryBook> findAllKeyedBy(
			Field fieldName, Order order, String filter) {
	...
	private static LibraryBook doConvertRow(Cursor cursor) {
		...
		newBook.setAuthor(new Author(cursor.getString(Field.a_first_name
				.ordinal()), cursor.getString(Field.a_last_name.ordinal())));
		...
		try {
			newBook.setAddedToLibrary(new Date(cursor.getLong(Field.date_added
					.ordinal())));
		} catch (RuntimeException r) {
		}

		try {
			newBook.setLastRead(new Date(cursor.getLong(Field.date_last_read
					.ordinal())));
		} catch (RuntimeException r) {
		}
