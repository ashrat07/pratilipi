...
import com.pratilipi.android.library.LibraryDatabaseHelper.Order;
...
public class SqlLiteLibraryService implements LibraryService {
	...
	public void storeBook(String fileName, Book book, boolean updateLastRead,
		...
		try {
			if (book.getCoverImage() != null
					&& book.getCoverImage().getSize() < MAX_COVER_SIZE) {
		...
		if (copyFile) {
			...
			Option<File> copiedFile = copyToLibrary(fileName, authorLastName
					+ ", " + authorFirstName, title);
		...
		this.helper.storeNewBook(bookFile.getAbsolutePath(), authorFirstName,
				authorLastName, title, description, thumbNail.unsafeGet(),
				updateLastRead);
	...
	private Option<File> copyToLibrary(String fileName, String author,
		...
		File targetFolder = new File(libraryFolder.unsafeGet(), cleanUp(author)
				+ "/" + cleanUp(title));
	...
	public Option<LibraryBook> getBook(String fileName) {
		QueryResult<LibraryBook> booksByFile = helper.findByField(
				LibraryDatabaseHelper.Field.file_name, fileName,
				LibraryDatabaseHelper.Field.file_name, Order.ASC, null);
	...
	public QueryResult<LibraryBook> findAllByAuthor(String filter) {
		return helper.findAllKeyedBy(LibraryDatabaseHelper.Field.a_last_name,
				LibraryDatabaseHelper.Order.ASC, filter);
	...
	public QueryResult<LibraryBook> findAllByLastAdded(String filter) {
		return helper.findAllOrderedBy(LibraryDatabaseHelper.Field.date_added,
				LibraryDatabaseHelper.Order.DESC, filter);
	}

	@Override
	public KeyedQueryResult<LibraryBook> findAllByTitle(String filter) {
		return helper.findAllKeyedBy(LibraryDatabaseHelper.Field.title,
				LibraryDatabaseHelper.Order.ASC, filter);
	}
	...
	@Override
	public void deleteBook(final String fileName) {
