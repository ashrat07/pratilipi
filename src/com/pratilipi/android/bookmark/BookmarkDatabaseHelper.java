 ...
 */
package com.pratilipi.android.bookmark;
...
public class BookmarkDatabaseHelper extends SQLiteOpenHelper {
	...
	public enum Field {
		file_name("TEXT NOT NULL"), name("TEXT NOT NULL"), book_index(
				"INTEGER NOT NULL"), book_position("INTEGER NOT NULL");
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL("CREATE UNIQUE INDEX fn_name_index ON " + TABLE_NAME + "("
				+ Field.file_name + ", " + Field.book_index + ", "
				+ Field.book_position + ");");

		for (int i = 0; i < 10000; i++) {

			Bookmark bm = new Bookmark("fake_file", Integer.toHexString(i),
					i % 3, i);
	...
	public List<Bookmark> getBookmarksForFile(String fileName) {
		...
		while (cursor.moveToNext()) {
			bookmarks.add(new Bookmark(cursor.getString(fileNameIndex), cursor
					.getString(nameIndex), cursor.getInt(indexIndex), cursor
